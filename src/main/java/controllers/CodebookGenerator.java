...
import services.CodeBookService;
...
public class CodebookGenerator {
    ...
    public static boolean generateCodeBook(int monthNo, int keySize) throws NoSuchAlgorithmException {
        ...
        int noKeysToGen = CodeBookService.daysInMonth(monthNo);

        // Generate array of keys for selected month
        String[] keyArr = CodeBookService.generateKeyArray(noKeysToGen, keySize);

        // Get name of chosen month
        String chosenMonth = CodeBookService.nameOfMonth(monthNo);
